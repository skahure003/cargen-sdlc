...
def render_inline(text: str, current_path: Path, metadata: dict):
    ...
    text = re.sub(
        r"\{\{[%<]\s*param\s+\"([^\"]+)\"\s*[>%]\}\}",
        lambda match: str(metadata.get(match.group(1), PARAMS.get(match.group(1), ""))),
        text,
    )
    text = re.sub(
        r"\{\{<\s*ref\s+\"([^\"]+)\"\s*>\}\}",
...
def render_markdown(body: str, current_path: Path, metadata: dict):
    ...
    body = re.sub(
        ...
        lambda match: (
            f'<figure><img src="/assets/{Path(match.group(1)).name}" '
    ...
    body = body.replace('src="/images/', 'src="/assets/')
    body = re.sub(
        r"\{\{<\s*ref\s+\"([^\"]+)\"\s*>\}\}",
        lambda match: resolve_ref(match.group(1), current_path),
        body,
    ...
    while index < len(lines):
        ...
        if line.startswith("<div class=\"hint"):
            ...
            inner = " ".join(part for part in block_lines[1:-1] if part)
            blocks.append(f"{opening}<p>{render_inline(inner, current_path, metadata)}</p></div>")
            continue
...
def load_site_content():
    ...
    for control in data["controls"]:
        controls_by_section.setdefault(control["section"], []).append(control)

    data["controls_by_section"] = controls_by_section
    data["background_lookup"] = {page["slug"]: page for page in data["background"]}
    data["area_lookup"] = {page["slug"]: page for page in data["areas"]}
    data["risk_lookup"] = {page["slug"]: page for page in data["risks"]}
    data["control_lookup"] = {
        (page["section"], page["slug"]): page for page in data["controls"]
    }
    return data
